...
python -m unittest discover -s tests  # 219 اختبار وحدة (بلا شبكة)
...
بعد أن أثبت الاختبار أن التنفيذ الآلي بلا حافة (`FINDINGS.md`)، صار المخرَج
الفعلي **موجز قرار** يقرؤه إنسان:
...
يُنتَج تلقائياً في كل دورة ذاتية أيضاً (`data/run1/briefing.md`).

لكل مرشح: السياق (فوليوم نسبي، الموقع من مدى 52 أسبوعاً، RSI)، والبنية
(الدعم وقمة الاختراق)، وحساب جاهز للوقف وحجم المركز بمخاطرة 1%، وقراءة
النظام مع تحذيراته.

### ثلاثة مبادئ تميّزه عن «التوصية»

1. **لا يتظاهر بما لا يعرف.** خطة البيانات تعطي السعر والفوليوم فقط، فيضع
   الموجز قائمة صريحة بما يجب أن تتحقق منه بنفسك: سبب الحركة، إفصاحات SEC،
   الفلوت الحقيقي، دفتر الأسعار. وهذه غالباً ما يحرّك السهم فعلاً — في حالة
   AEMD كان اندماجاً وتجزئة عكسية وطرحاً تمويلياً، ولا شيء منها في السعر.

2. **الفلاتر تحذيرات لا أحكام.** ما دام القرار بشرياً فحجب المعلومة خطأ:
   `require_known_float=False` في المسار البشري يعرض الرمز مع تحذير «الفلوت
   غير معروف» بدل إسقاطه. المسار الآلي يبقى فاشلاً بأمان (يرفض عند الجهل).

3. **الانضباط الذي نجا من الاختبار ملزم**: لا تلاحق قفزة، لا تخفّف فلتر
   السيولة (تخفيفه أنزل التوقع من +0.100R إلى −0.017R)، لا تشترِ من العرض،
   وحجم واحد لكل صفقة.

## التشغيل الذاتي الكامل

```bash
echo "TWELVE_DATA_API_KEY=xxxx" > .env    # غير مرفوع للمستودع
./autorun.sh install                       # مهمة يومية 02:05 UTC — بعدها لا تدخّل
./autorun.sh status                        # ماذا جرى وماذا اعتُمد
```

كل دورة: تدفع خط الإنتاج ضمن ميزانية اليوم، وعند اكتمال الاختبار تعيد الضبط
والتحقق خارج العينة، ثم تقرر عبر **بوابة الترقية**.

### بوابة الترقية — جوهر الأمان

التشغيل الذاتي بلا بوابة يصبح **آلة لاصطياد الضوضاء**: يجرّب حتى يجد رقماً
جميلاً بالصدفة ثم يتبنّاه. الشروط الأربعة لاعتماد أي إعداد جديد:

1. يتفوق على الإعداد الحالي **داخل العينة** (أقدم 60% من التاريخ).
2. عينته ≥ 60 صفقة.
3. يصمد **خارج العينة** (آخر 40% — لم يره الضبط إطلاقاً).
4. مجال ثقته هناك **موجب بالكامل**، بمستوى مصحَّح بـ Bonferroni لعدد التجارب.

يسقط شرط واحد ⇒ يبقى الإعداد الحالي، ويُسجَّل الرفض في `auto_state.json`.
بخمس تجارب افتراضية يصبح المستوى المطلوب 99% بدل 95%.

### ما لا يؤتمت عمداً: إرسال الأوامر

الاختبار على 113 صفقة أعطى توقعاً لا يُميَّز عن الصفر، وانقلب سالباً خارج
العينة (`FINDINGS.md`). ربط تنفيذ آلي باستراتيجية بهذه النتيجة تحويلٌ لرأس
المال إلى ضوضاء. المخرَج تنبيهات يقرؤها إنسان — والقرار قراره.
...
trading_bot/
  ...
  briefing.py                        الموجز اليومي (المخرَج البشري)
  briefing_cli.py                    سطر أوامر الموجز
  strategies/momentum_breakout.py    الفرضية المضادة: شراء القوة
  auto.py                            التشغيل الذاتي وبوابة الترقية
  auto_cli.py                        سطر أوامر التشغيل الذاتي
  sweep.py                           الضبط محمياً من التفصيل على المقاس
  FINDINGS.md                        نتائج الاختبار الفعلي (أرقام لا آراء)
...
tests/test_auto.py                   14 اختبار للتشغيل الذاتي وبوابته
tests/test_momentum.py               11 اختبار للفرضية المضادة
tests/test_briefing.py               14 اختبار للموجز اليومي
```
